...
import static com.netflix.conductorboot.lib.embedded.elastic.Require.require;
...
import com.netflix.conductorboot.lib.embedded.elastic.InstallationDescription.Plugin;
...
public final class EmbeddedElastic {
     ...
     */
    public synchronized EmbeddedElastic start(boolean force_installation) throws IOException, InterruptedException {
        if (!started) {
            started = true;
        	installElastic(force_installation);
            startElastic();
            createRestClient();
            createTemplates();
            createIndices();
    ...
    private void installElastic(boolean force_installation) throws IOException, InterruptedException {
        ElasticSearchInstaller elasticSearchInstaller = new ElasticSearchInstaller(instanceSettings, installationDescription);
        if(!elasticSearchInstaller.getInstallationDirectory().exists() || !elasticSearchInstaller.getExecutableFile().exists() || force_installation)
    ...
    public synchronized boolean isElasticServerStopped() {
    ...
    public static final class Builder {
        ...
        public EmbeddedElastic build() {
            require(installationSource != null, "You must specify elasticsearch version, or download url or Home Folder");
